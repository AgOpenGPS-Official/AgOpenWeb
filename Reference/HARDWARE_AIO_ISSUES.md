...
| **P3** | L4 | **Ethernet pairs likely ≈ 66 Ω differential instead of 100 Ω.** | `ETH` rule width 0.377 mm with 0.10 mm gap (necking to 0.2/0.15 mm at pins). IPC-2141 estimate on JLC 1.6 mm 4-layer 7628 (L1–L2 ≈ 0.21 mm, εr ≈ 4.4): ≈ 48 Ω single-ended / ≈ 66 Ω differential. 0.377 mm looks like a 50 Ω single-ended width. PCIe (0.226/0.103 mm) estimates ≈ 89 Ω, which is OK. | Confirm with JLC's impedance calculator for the ordered stackup; set the `ETH` rule to the 100 Ω differential width/gap; re-route (all on L1, no vias). | **open** — 09-18 re-route is now a uniform **0.200 mm width, ~0.20 mm gap, single layer, no vias** (was 0.377/0.10 with necks to 0.15). Estimated ~109 Ω, so slightly high rather than low: widen to **0.23 mm** or tighten the gap to **0.15 mm** for ~102 Ω. Confirm against JLC's calculator for the ordered stackup. |
