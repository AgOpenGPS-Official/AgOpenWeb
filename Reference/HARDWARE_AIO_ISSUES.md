...
|---|---|---|---|---|---|
| **P1** | L1 | Input TVS on a thin trace. | Was 0.254 mm × 4.8 mm to C6. | **Mostly fixed 2026-09-17:** TV1 pad 1 now has a **1.5 mm** trace to `VIN_PROT`. **Still open:** pad 2 (GND) has no trace and no vias — it relies on the top pour alone. A clamp sinking ~15 A needs 2–3 vias in/beside the ground pad straight to the inner plane; worth doing now rather than with the deferred P2 stitching. | **part-fixed** |
